...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'IPInfo');
}
...
async function lookupIp(ip: string) {
  const res = await pwFetch(`${BASE_URL}/${encodeURIComponent(ip)}/json`);
  if (!res.ok) throw await httpError(res, 'ipinfo.io error');
  const data = (await res.json()) as IpInfoResponse;
  return formatResponse(data);
}

async function getMyIp() {
  const res = await pwFetch(`${BASE_URL}/json`);
  if (!res.ok) throw await httpError(res, 'ipinfo.io error');
  const data = (await res.json()) as IpInfoResponse;
  return formatResponse(data);
}
